...
(function(a){a.extend(a.fn.cycle.defaults,{loader:false});a(document).on("cycle-bootstrap",function(d,b){var f;if(!b.loader){return}f=b.API.add;b.API.add=c;function c(l,j){var h=[];if(typeof l=="string"){l=l.trim()}else{if(typeof l==="array"){for(var k=0;k<l.length;k++){l[k]=a(l[k])[0]}}}l=a(l);var g=l.length;if(!g){return}l.css("visibility","hidden").appendTo("body").each(function(q){var r=0;var o=a(this);var n=o.is("img")?o:o.find("img");o.data("index",q);n=n.filter(":not(.cycle-loader-ignore)").filter(':not([src=""])');if(!n.length){--g;h.push(o);return}r=n.length;n.each(function(){if(this.complete){p()}else{a(this).on("load",function(){p()}).on("error",function(){if(--r===0){b.API.log("slide skipped; img not loaded:",this.src);if(--g===0&&b.loader=="wait"){f.apply(b.API,[h,j])}}})}});function p(){if(--r===0){--g;e(o)}}});if(g){b.container.addClass("cycle-loading")}function e(i){var n;if(b.loader=="wait"){h.push(i);if(g===0){h.sort(m);f.apply(b.API,[h,j]);b.container.removeClass("cycle-loading")}}else{n=a(b.slides[b.currSlide]);f.apply(b.API,[i,j]);n.show();b.container.removeClass("cycle-loading")}}function m(n,i){return n.data("index")-i.data("index")}}})})(jQuery);
